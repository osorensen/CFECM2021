...
$\zeta_{k}^{(3)}, ~~ k=1,\dots,K,$ are inverse smooothing parameters for $K$ regression splines.
...
- Profile likelihood algorithm used here is convenient, but does not scale very well.
...
- When the grouping structure is strictly hierarchical, it is possible to integrate out the latent variables from the likelihood function using adaptive quadrature rules<sup>1</sup>.
  ...
  - We are currently working on formulating adaptive integration as a series of operations on sparse matrices. This will allow faster estimation, and also more accurate, as we don't have to rely on the Laplace approximation.
  

.footnote[[1] [Rabe-Hesketh, Skrondal, and Pickles (2005)](https://doi.org/10.1016/j.jeconom.2004.08.017)]  
